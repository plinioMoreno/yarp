...
void PortCoreInputUnit::run() {
    ...
    if (autoHandshake) {
        ...
        if (!ok) {
            ...
            if (Name(route.getFromName()).isRooted()) {
                yDebugNoFw("Port %s starting up, flushing routes %s->*->%s",
    ...
    while (!done) {
        ...
        switch (key) {
        case '/':
            yDebugNoFw("Port command (%s): %s should add connection: %s",
            ...
            break;
        case '!':
            yDebugNoFw("Port command (%s): %s should remove output: %s",
                       route.toString().c_str(),
                       getOwner().getName().c_str(),
                       cmd.getText().c_str());
            ...
            break;
        case '~':
            yDebugNoFw("Port command (%s): %s should remove input: %s",
                       route.toString().c_str(),
                       getOwner().getName().c_str(),
                       cmd.getText().c_str());
