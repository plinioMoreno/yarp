...
bool RpcServer::write(PortWriter& writer, PortWriter *callback) const {
    yErrorNoFw("RpcServer %s cannot write, please use a regular Port or RpcClient for that", port.getName().c_str());
    return false;
}
...
bool RpcServer::read(PortReader& reader, bool willReply) {
    if (!willReply) {
        yErrorNoFw("RpcServer %s must reply, please use a regular Port if you do not want to", port.getName().c_str());
